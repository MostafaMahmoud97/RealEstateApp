<?php

return [

    "Type identities have been fetched" => "تم جلب نوع الهويات",
    "Client has been created success" => "تم انشاء عميل جديد بنجاح",
    "Clients have been fetched success" => "تم جلب العملاء بنجاح",
    "no client by this id" => "الا يوجد عميل بهذا ال id",
    "Client has been fetched success" => "تم جلب العميل بنجاح",
    "Client has been updated success" => "تم تعديل العميل بنجاح",
    "Client has been reset password success" => "تم اعادة ضبط كلمة المرور بنجاح",

    "you must choose type id" => "يجب ادخال نوع الهوية",
    "you must enter number in type id" => "يجب ادخال رقم فى نوع الهوية",
    "you must enter number in type id as 1,2,3" => "يجب عليك إدخال الرقم في معرف النوع كـ 1،2،3",
    "you must enter full name" => "يجب ادخال الاسم بالكامل",
    "you must enter string in full name" => "يجب ادخال الاسم صحيحاً",
    "you must enter nationality" => "يجب ادخال الجنسية",
    "you must enter string in nationality" => "يجب ادخال الجنسية صحيحة",
    "you must enter id number" => "يجب ادخال رقم الهوية",
    "you must enter number in id number" => "يجب ادخال ارقام فى رقم الهوية",
    "you must enter phone number" => "يجب ادخال رقم الجوال",
    "you must enter string in phone number" => "يجب ادخال رقم الجوال صحيحاُ",
    "This mobile number is already in use" => "رقم الجوال هذا مستخدم من قبل",
    "you must enter email" => "يجب ادخال الايميل",
    "you must enter valid email with @ sign" => "يجب ادخال الايميل صحيحاً مع علامة @",
    "This email is already in use" => "هذا الايميل مستخدم من قبل",
    "you must enter password" => "يجب ادخال البسورد",
    "password confirmation not match with password" => "تاكيد البسورد لا يتطبق مع البسورد",
    "you must enter min:6 characters in password" => "يجب ان لا يقل البسورد عن 6 حروف",

];
